...
H2w = tf.Variable(tf.random_normal([memshape,memshape])) 
H2b = tf.Variable(tf.random_normal([memshape])) 
M1w = tf.Variable(tf.random_normal([memshape+fixshape, fixshape])) 
...
def runnfixes(image,H1m_old,H1m_diff_RollingAverage):
    ...
    return fix, H1m_diff_RollingAverage, M1a, H1m_diff
