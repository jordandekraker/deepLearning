...
topweights = tf.Variable(tf.random_normal([inputshape+fixshape+fixshape, 10+fixshape])) 
...
def runnfixes(image,memstate,meanmemdiff):
    ...
    for n in range(nfixes-1):  
        ...
        l2withfix = tf.reshape(tf.concat((l2[0,:],fix[n,]),0),[1,inputshape+fixshape+fixshape])
        l3 = tf.matmul(l2withfix, topweights) + topbiases # linear activation function?
    ...
    return fix, memstate, l2, meanmemdiff,n
...
for i in range(iters): # iterations
    ...
    lossperiter[i],accuracyperiter[i],finalfix,meanmdiff[i,:] = sess.run(
            [loss_MNIST,accuracy_MNIST,seqfix,mdiff], 
